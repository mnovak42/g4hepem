 ...
 */
// --------------------------------------------------------------------
#ifndef G4HepEmLog_HH
#define G4HepEmLog_HH 1
...
inline float VDTLogf(float x) {
  const float original_x = x;

  float fe;
  x = DVTLogConsts::getMantExponentf(x, fe);

  x > DVTLogConsts::SQRTHF ? fe += 1.f : x += x;
  x -= 1.0f;

  const float x2 = x * x;

  float res = DVTLogConsts::get_log_poly(x);
  res *= x2 * x;
  ...
  if(original_x > DVTLogConsts::LOGF_UPPER_LIMIT)
    res = std::numeric_limits<float>::infinity();
  if(original_x < DVTLogConsts::LOGF_LOWER_LIMIT)
