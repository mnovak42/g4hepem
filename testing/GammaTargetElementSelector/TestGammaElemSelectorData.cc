...
//  theCoupleTable->DumpCouples();
  ...
  if ( !TestGammaElemSelectorData( runMgr->GetHepEmData() ) ) {
    return 1;
  } else if ( verbose > 0 ) {
#ifdef G4HepEm_CUDA_BUILD
    std::cout << " === Target Element Selector Test for Gamma: PASSING (HepEm HOST v.s. DEVICE) \n" << std::endl;
#else   // G4HepEm_CUDA_BUILD
    std::cout << " === Target Element Selector Test for Gamma: PASSING (HepEm HOST) \n" << std::endl;
#endif  // G4HepEm_CUDA_BUILD
  }
